...
def shell():
    ...
    c = c + 'Retrieving speedtest.net configuration...' +"\n\n"
    ...
    try:
        ...
        c = c + 'Cannot retrieve speedtest configuration' + "\n"
    ...
    if args.list:
        try:
            ...
            c = c+ 'Cannot retrieve speedtest server list' +"\n"
            ...
            for server in servers:
                ...
                try:
                    ...
                    if e.errno != errno.EPIPE:
                        raise
        sys.exit(0)
    c+= 'Testing from %(isp)s (%(ip)s)...' % speedtest.config['client'] +"\n\n"
    # c+='Testing from '
    # c+="<span style=\" font-size:8pt; font-weight:600; color:#ff0000;\" >"
    # c+=' %(isp)s (%(ip)s)...' % speedtest.config['client']
    # c+="</span>"
    # c+="\n"
    ...
    if not args.mini:
        c = c + 'Retrieving speedtest.net server list...' +"\n"
        ...
        if args.server and len(args.server) == 1:
            c=c+'Retrieving information for the selected server...'+"\n"
            printer('Retrieving information for the selected server...', quiet)
        else:
            c=c+'Selecting best server based on ping...'+"\n"
    ...
    c=c+'Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: ''%(latency)s ms' % results.server +"\n\n"
    printer('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
            '%(latency)s ms' % results.server, quiet)

    if args.download:
        c=c+'Testing download speed...........................'+"\n"
        ...
        c = c + 'Download: %0.2f M%s/s' %((results.download / 1000.0 / 1000.0) / args.units[1],args.units[0]) + "\n\n"
        printer('Download: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        c=c+'Skipping download test'+"\n"
        printer('Skipping download test', quiet)

    if args.upload:
        c=c+'Testing upload speed......................................'+ "\n"
        ...
        c=c+'Upload: %0.2f M%s/s' %((results.upload / 1000.0 / 1000.0) / args.units[1],args.units[0])+"\n\n"
        printer('Upload: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        c=c+'Skipping upload test'+"\n"
    ...
    if args.simple:
        ...
        c=c+results.csv(delimiter=args.csv_delimiter)+"\n"
    ...
    if args.share and not machine_format:
        c=c+'Share results: %s' % results.share()+"\n"
